...
test.describe('Plans gallery feature test suite', () => {
    ...
    test(`[Test Id - ${features[0].tcid}] ${features[0].name},${features[0].tags}`, async () => {
        ...
        await test.step('step-1: Go to Plans gallery page', async () => {
            const page = workerSetup.getPage('US');
            galleryPage = new MasPlans(page);
            await workerSetup.verifyPageURL('US', DOCS_GALLERY_PATH.PLANS.US, expect);
        });

        await test.step('step-2: Verify Plans card content', async () => {
    ...
    test(`[Test Id - ${features[2].tcid}] ${features[2].name},${features[2].tags}`, async () => {
        ...
        await test.step('step-3: Verify cards without badge have no empty vertical gap at top', async () => {
            const allCards = workerSetup.getPage('US').locator('.four-merch-cards.plans merch-card');
